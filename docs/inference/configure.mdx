...
openshell provider create --name vertex-prod --type vertex --from-existing
```

This reads `ANTHROPIC_VERTEX_PROJECT_ID`, optional
`ANTHROPIC_VERTEX_REGION`, and Google Application Default Credentials from
`VERTEX_ADC` or `~/.config/gcloud/application_default_credentials.json`.
OpenShell refreshes the short-lived Vertex bearer token automatically when it
builds the managed route.
...
For Vertex AI, use the Anthropic model identifier without a base URL:
...
The gateway constructs the Vertex endpoint from
`ANTHROPIC_VERTEX_PROJECT_ID` and `ANTHROPIC_VERTEX_REGION`, rewrites
`/v1/messages` requests to Vertex's
`.../publishers/anthropic/models/<model>:streamRawPredict` path, and injects
the required `anthropic-version: vertex-2023-10-16` header.
