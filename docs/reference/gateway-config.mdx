...
# Disable to prevent the gateway from adding a non-loopback callback listener
# for rootless Podman pasta when the primary bind address is loopback-only.
...
`enable_auto_callback_listener` defaults to `true`. When rootless Podman uses
pasta and the gateway bind address is loopback-only, the driver may add a
bindability-checked non-loopback callback listener if TLS client certificate
authentication is enabled. Set it to `false`, or set
`OPENSHELL_PODMAN_ENABLE_AUTO_CALLBACK_LISTENER=false`,
to opt out.
