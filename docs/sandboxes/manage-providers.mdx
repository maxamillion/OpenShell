...
### Vertex AI from Local ADC

Use the `vertex` provider type when you want Claude Code inside the sandbox to
authenticate through Google Cloud Vertex AI.
...
openshell provider create --name my-vertex --type vertex --from-existing
```

`--from-existing` reads:

- `ANTHROPIC_VERTEX_PROJECT_ID`
- `ANTHROPIC_VERTEX_REGION` when set
- Google Application Default Credentials from `VERTEX_ADC` or `~/.config/gcloud/application_default_credentials.json`
...
| `vertex` | `ANTHROPIC_VERTEX_PROJECT_ID`, `ANTHROPIC_VERTEX_REGION`, `CLAUDE_CODE_USE_VERTEX` visible to child processes; `VERTEX_ADC` and `VERTEX_ACCESS_TOKEN` stay supervisor-only | Claude Code with Google Cloud Vertex AI, or `inference.local` routed to Vertex |
...
Two supported patterns use the same `vertex` provider type.

### Direct Sandbox Use with Claude Code

Create the provider from local ADC:
...
Attach it when you create the sandbox:

```shell
openshell sandbox create --provider my-vertex -- claude
```

Inside the sandbox, OpenShell projects `ANTHROPIC_VERTEX_PROJECT_ID`,
`ANTHROPIC_VERTEX_REGION`, and `CLAUDE_CODE_USE_VERTEX=1` into the child
process environment. The supervisor also creates a fake
`~/.config/gcloud/application_default_credentials.json` file and intercepts the
Google OAuth token exchange that Claude Code performs. The real ADC JSON and
refreshed Vertex access token stay in supervisor memory and are not exposed to
the sandboxed process.

Allow these external endpoints in your sandbox policy when Claude Code talks to
Vertex directly:

- `oauth2.googleapis.com:443`
- `<region>-aiplatform.googleapis.com:443`

### `inference.local` Routed Through Vertex

Use the same provider with the managed local endpoint when you want all sandbox
code to call `https://inference.local` instead of reaching Vertex directly.
...
In this mode the gateway refreshes the Vertex bearer token, rewrites the model
into the Vertex URL shape, and forwards Anthropic-compatible requests to the
correct Google Cloud regional endpoint.
