...
ensure_local_registry() {
	if $CONTAINER_RUNTIME inspect "${LOCAL_REGISTRY_CONTAINER}" >/dev/null 2>&1; then
		local proxy_remote_url
		proxy_remote_url=$($CONTAINER_RUNTIME inspect "${LOCAL_REGISTRY_CONTAINER}" --format '{{range .Config.Env}}{{println .}}{{end}}' 2>/dev/null | awk -F= '/^REGISTRY_PROXY_REMOTEURL=/{print $2; exit}' || true)
		if [ -n "${proxy_remote_url}" ]; then
			$CONTAINER_RUNTIME rm -f "${LOCAL_REGISTRY_CONTAINER}" >/dev/null 2>&1 || true
		fi
	fi

	if ! $CONTAINER_RUNTIME inspect "${LOCAL_REGISTRY_CONTAINER}" >/dev/null 2>&1; then
		$CONTAINER_RUNTIME run -d --restart=always --name "${LOCAL_REGISTRY_CONTAINER}" -p 5000:5000 registry:2 >/dev/null
	else
		if ! $CONTAINER_RUNTIME ps --filter "name=^${LOCAL_REGISTRY_CONTAINER}$" --filter "status=running" -q | grep -q .; then
			$CONTAINER_RUNTIME start "${LOCAL_REGISTRY_CONTAINER}" >/dev/null
		fi

		port_map=$($CONTAINER_RUNTIME port "${LOCAL_REGISTRY_CONTAINER}" 5000/tcp 2>/dev/null || true)
		case "${port_map}" in
		*:5000*) ;;
		*)
			$CONTAINER_RUNTIME rm -f "${LOCAL_REGISTRY_CONTAINER}" >/dev/null 2>&1 || true
	...
	echo "       Ensure a registry is running on port 5000 (e.g. $CONTAINER_RUNTIME run -d --name openshell-local-registry -p 5000:5000 registry:2)." >&2
	$CONTAINER_RUNTIME ps -a >&2 || true
	$CONTAINER_RUNTIME logs "${LOCAL_REGISTRY_CONTAINER}" >&2 || true
	exit 1
}
...
if [ -n "${CI:-}" ] && [ -n "${CI_REGISTRY:-}" ] && [ -n "${CI_REGISTRY_USER:-}" ] && [ -n "${CI_REGISTRY_PASSWORD:-}" ]; then
	printf '%s' "${CI_REGISTRY_PASSWORD}" | $CONTAINER_RUNTIME login -u "${CI_REGISTRY_USER}" --password-stdin "${CI_REGISTRY}"
...
if [ "${MODE}" = "fast" ]; then
	if $CONTAINER_RUNTIME inspect "${CONTAINER_NAME}" >/dev/null 2>&1 || $CONTAINER_RUNTIME volume inspect "${VOLUME_NAME}" >/dev/null 2>&1; then
...
if [ -n "${GATEWAY_HOST:-}" ]; then
	...
	if ! getent hosts "${GATEWAY_HOST}" >/dev/null 2>&1; then
		BRIDGE_IP=$($CONTAINER_RUNTIME network inspect bridge --format '{{(index .IPAM.Config 0).Gateway}}' 2>/dev/null || true)
		if [ -n "${BRIDGE_IP}" ]; then
